...

{
  ...
  var qmSettings = {
    load: function () {
      ...
      let createElement = document.createXULElement
      ...
      for (let i = 0; i < myAccounts.length; i++) {
        ...
        if (server && server.canFileMessagesOnServer) {
          ...
          chk.addEventListener("click", function (evt) {
            qmSettings.toggleAccountExclusion(chk, evt);
          });
          chk.addEventListener("keydown", function (evt) {
            qmSettings.toggleAccountExclusion(chk, evt);
          });
      ...
      chkLock.addEventListener("click", function (evt) {
        qmSettings.toggleLockInAccount(chkLock, evt);
      });
      chkLock.addEventListener("keydown", function (evt) {
        qmSettings.toggleLockInAccount(chkLock, evt);
      });

      let chkSilent = document.getElementById("chkSilent");
      chkSilent.checked = globalQuickMoveSettings.isSilent;
      chkSilent.addEventListener("click", function (evt) {
        qmSettings.toggleSilent(chkSilent, evt);
      });
      chkSilent.addEventListener("keydown", function (evt) {
        qmSettings.toggleSilent(chkSilent, evt);
      });

      let chkEsc = document.getElementById("chkEscapeClearsList");
      chkEsc.checked = globalQuickMoveSettings.isClearList;
      chkEsc.addEventListener("click", function (evt) {
        qmSettings.toggleClearList(chkEsc, evt);
      });
      chkEsc.addEventListener("keydown", function (evt) {
        qmSettings.toggleClearList(chkEsc, evt);
      });
      ...
      chkCollapse.addEventListener("click", function (evt) {
        qmSettings.toggleCollapsed(chkCollapse, evt);
      });
      chkCollapse.addEventListener("keydown", function (evt) {
        qmSettings.toggleCollapsed(chkCollapse, evt);
      });

      let chkGoNext = document.getElementById("chkGoNext");
      chkGoNext.checked = globalQuickMoveSettings.isGoNext;
      chkGoNext.addEventListener("click", function (evt) {
        qmSettings.toggleGoNext(chkGoNext, evt);
      });
      chkGoNext.addEventListener("keydown", function (evt) {
        qmSettings.toggleGoNext(chkGoNext, evt);
      });

      let chkReopen = document.getElementById("chkReopen");
      chkReopen.checked = globalQuickMoveSettings.isReopen;
      chkReopen.addEventListener("click", function (evt) {
        qmSettings.toggleReopen(chkReopen, evt);
      });
      chkReopen.addEventListener("keydown", function (evt) {
        qmSettings.toggleReopen(chkReopen, evt);
      });
